...
pub async fn add_device(device: Device) -> Result<Value, String> {
        ...
        .json(&json!(device))
    {
        if let Ok(response) = builder.send().await {
            if let Ok(text) = response.text().await {
                ...
                Err(String::from(
                    "Empty API response. The API might not be available.",
                ))
            }
        } else {
            Err(String::from(
                "Request failed. The API might not be available.",
            ))
        }
    } else {
        Err(String::from(
            "Failed to create request. Cannot add new device.",
        ))
    }
}
...
pub async fn upload_connectivity_files(
    ...
    let form_data = FormData::new().map_err(|e| {
        e.as_string()
            .unwrap_or("Error al crear FormData".to_string())
    })?;

    // Agregar los archivos a FormData
    if let Some(context) = complete_context {
        form_data
            .append_with_blob("complete_context_file", &context)
            .map_err(|e| {
                e.as_string()
                    .unwrap_or("Error al agregar el archivo de Complete Context".to_string())
            })?;
    } else {
        form_data
            .append_with_blob("topology_file", &topology.unwrap())
            .map_err(|e| {
                e.as_string()
                    .unwrap_or("Error al agregar el archivo de Topology".to_string())
            })?;
        form_data
            .append_with_blob("connections_file", &connections.unwrap())
            .map_err(|e| {
                e.as_string()
                    .unwrap_or("Error al agregar el archivo de Connections".to_string())
            })?;
        form_data
            .append_with_blob(
                "connectivity_services_file",
                &connectivity_services.unwrap(),
            )
            .map_err(|e| {
                e.as_string()
                    .unwrap_or("Error al agregar el archivo de Connectivity Services".to_string())
            })?;
    }
    ...
    let json_blob_result =
        Blob::new_with_str_sequence_and_options(&json_jsvalue_array, &blob_options);
    let json_blob = json_blob_result.unwrap();
    // Agregar el Blob JSON al FormData
    form_data
        .append_with_blob("json", &json_blob)
        .map_err(|e| {
            e.as_string()
                .unwrap_or("Error al agregar el JSON al FormData".to_string())
        })?;
