...
async fn save_schema() -> Result<HttpResponse, Error> {
    ...
    let json: Value = response
        .json()
        .await
        .map_err(|_| error::ErrorNotFound("Empty Response."))?;
    ...
    let connectivity_services = json
        ...
        .as_array()
        .ok_or_else(|| error::ErrorNotAcceptable("connectivity-service is not and array"))?
        .clone();
    let connections = json
        ...
        .as_array()
        .ok_or_else(|| error::ErrorNotAcceptable("connection is not and array"))?
        .clone();
    let services_interface_point = json
        .pointer("/tapi-common:context/service-interface-point")
        .ok_or_else(|| error::ErrorNotFound("Cannot find service-interface-point"))?
        .as_array()
        .ok_or_else(|| error::ErrorNotAcceptable("service-interface-point is not and array"))?
        .clone();
    let topology = json
        .pointer("/tapi-common:context/tapi-topology:topology-context/topology")
        .ok_or_else(|| error::ErrorNotFound("Cannot find topology"))?
        .clone();

    // Serialize each part into a JSON string
    let connectivity_services_str = serde_json::to_string(&connectivity_services)
        .map_err(|_| error::ErrorInternalServerError("Serialization Error"))?;
    let connections_str = serde_json::to_string(&connections)
        .map_err(|_| error::ErrorInternalServerError("Serialization Error"))?;
    let services_interface_point_str = serde_json::to_string(&services_interface_point)
        .map_err(|_| error::ErrorInternalServerError("Serialization Error"))?;
    let topology_str = serde_json::to_string(&topology)
        .map_err(|_| error::ErrorInternalServerError("Serialization Error"))?;

    // Write each JSON string to a separate file
    let mut file = File::create("connectivity_services.json")
        .map_err(|_| error::ErrorInternalServerError("File Creation Error"))?;
    file.write_all(connectivity_services_str.as_bytes())
        .map_err(|_| error::ErrorInternalServerError("File Write Error"))?;

    let mut file = File::create("connections.json")
        .map_err(|_| error::ErrorInternalServerError("File Creation Error"))?;
    file.write_all(connections_str.as_bytes())
        .map_err(|_| error::ErrorInternalServerError("File Write Error"))?;

    let mut file = File::create("services_interface_point.json")
        .map_err(|_| error::ErrorInternalServerError("File Creation Error"))?;
    file.write_all(services_interface_point_str.as_bytes())
        .map_err(|_| error::ErrorInternalServerError("File Write Error"))?;

    let mut file = File::create("topology.json")
        .map_err(|_| error::ErrorInternalServerError("File Creation Error"))?;
    file.write_all(topology_str.as_bytes())
        .map_err(|_| error::ErrorInternalServerError("File Write Error"))?;
